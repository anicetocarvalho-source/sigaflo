...
import { useCoffeeLots, useCoffeeExporters, CoffeeLot } from '@/hooks/useCoffee';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { Skeleton } from '@/components/ui/skeleton';
import { CoffeeLotForm } from './CoffeeLotForm';
import { CoffeeLotDetail } from './CoffeeLotDetail';
import { CoffeeLotsCharts } from './CoffeeLotsCharts';
import { CoffeeLotsAlerts } from './CoffeeLotsAlerts';
import { CoffeeLotsMap } from './CoffeeLotsMap';
...
export function CoffeeLotsDashboard() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Código', 'Origem', 'Nº Produtores', 'Volume (kg)', 'Sacas', 'Variedade', 'Qualidade', 'Estado', 'Exportador', 'Destino'];
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <CardHeader>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription>
                    Gestão e rastreabilidade de lotes ({lots?.length || 0} registos)
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <Button className="mt-4" onClick={handleNewLot}>
                    <Plus className="mr-2 h-4 w-4" />
                    Criar Primeiro Lote
                  </Button>
                </div>
                          ...
                          >
                            ...
                            <TableCell className="text-right">
                              <div>
                                ...
                                {lot.bags_count && lot.bags_count > 0 && (
                            ...
                            <TableCell>
                              <span className="text-sm">{lot.variety || '-'}</span>
                            </TableCell>
                            <TableCell>
          ...
          {lots && lots.length > 0 ? (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-muted-foreground">
                  Adicione lotes de café para ver estatísticas
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="mapa" className="mt-4">
          {lots && lots.length > 0 ? (
            <CoffeeLotsMap lots={lots} onSelectLot={handleSelectLot} />
          ) : (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                <MapPin className="h-12 w-12 text-muted-foreground/50 mb-4" />
                <p className="text-lg font-medium">Sem dados geográficos</p>
                <p className="text-sm text-muted-foreground">
                  Adicione lotes com origem para visualizar no mapa
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="alertas" className="mt-4">
          {lots ? (
            <CoffeeLotsAlerts lots={lots} onSelectLot={handleSelectLot} />
          ) : (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                <Bell className="h-12 w-12 text-muted-foreground/50 mb-4" />
                <p className="text-lg font-medium">Carregando alertas...</p>
              </CardContent>
            </Card>
          )}
        </TabsContent>
